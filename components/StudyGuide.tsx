...
const StudyGuide: React.FC<StudyGuideProps> = ({ markdown }) => {
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      // PRE-PROCESSING: Clean markdown for plain text PDF
      // jsPDF (default fonts) does not support Emojis, so we must strip them to avoid errors.
      let text = markdown
        // 1. Convert List Items (* or -) to bullets
        .replace(/^\s*[\-\*]\s+/gm, '• ')
        ...
        .replace(/^#+\s+/gm, '')
        // 4. Clean Links: [Text](url) -> Text (url)
        .replace(/\[([^\]]+)\]\(([^\)]+)\)/g, '$1 ($2)')
        // 5. Remove Code ticks
        .replace(/`{1,3}/g, '')
        // 6. STRIP EMOJIS (Critical for jsPDF stability)
        .replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1F018}-\u{1F270}\u{1F910}-\u{1F96B}\u{1F980}-\u{1F9E0}]/gu, '');
      ...
      doc.text("Study Guide", margin, 20);
      
      // Add Timestamp
      doc.setFontSize(10);
      doc.setFont("helvetica", "italic");
      doc.setTextColor(100);
      doc.text(`Generated by NoteCraft on ${new Date().toLocaleDateString()}`, margin, 27);
      ...
      doc.save("NoteCraft-StudyGuide.pdf");
    } catch (error) {
      console.error("PDF Generation Error:", error);
      alert("Could not generate PDF. Please use the Copy button instead.");
    }
  };
